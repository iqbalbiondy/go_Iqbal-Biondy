...
func InitDb() {
	...
	connectionString := "root:iqbal@tcp(localhost:3306)/porto?charset=utf8&parseTime=True&loc=Local"
	var err error
...
func InitDBTest() {
	//connectionString := os.Getenv("CONNECTION_STRING")
	connectionString := "root:iqbal@tcp(localhost:3306)/porto_test?charset=utf8&parseTime=True&loc=Local"
	var err error
	DB, err = gorm.Open(mysql.Open(connectionString), &gorm.Config{})
	if err != nil {
		panic(err)
